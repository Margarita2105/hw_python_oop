...
class CaloriesCalculator(Calculator):
    ...
    def get_calories_remained(self):
        self.lim = self.limit - super().get_today_stats()
        if self.lim <= 0:
            self.answer = 'Хватит есть!'
        else:
            self.answer = 'Сегодня можно съесть что-нибудь ещё, но с общей калорийностью не более ' f'{self.lim}' ' кКал'
        return self.answer
class CashCalculator(Calculator):
    ...
    def get_today_cash_remained(self, currency):
        ...
        if self.lim == 0:
            ...
            self.answer = 'Денег нет, держись: твой долг - ' f'{- float(self.c)} {cur1[currency]}'
        else:
            if currency == 'usd':
